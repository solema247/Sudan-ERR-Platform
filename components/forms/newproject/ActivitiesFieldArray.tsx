...
const ActivitiesFieldArray = ({ optionsActivities, optionsExpenses }) => (
                ...
                {({ push, remove }) => (
                  <div className="space-y-2">
                    {activity.expenses.map((expense, expenseIndex) => (
                      <div key={expenseIndex} className="flex space-x-2">
                        <Field
                          name={`planned_activities[${index}].expenses[${expenseIndex}].description`}
                          type="text"
                          placeholder="Expense Description"
                          className="text-sm w-1/2 p-2 border rounded-lg"
                        />
                        <Field
